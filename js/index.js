...
{
	Buffer = [];
	for (let i = 2; i < hex.length; i=i+2) {
		nextchar = hex.charAt(i) + hex.charAt(i+1);
		Buffer[i/2-1]='0x'+nextchar;
	}
	return Buffer
}

function getaddress(npubsec,hex){
	var kwords = bech32.toWords(hex_decode(hex))
	pub=bech32.encode(npubsec, kwords)
	return pub;
}
        function calculateValues() {
            setTimeout(function() {
                ...
                if (networkIsEthereum()) {
					...
					pubkey = getaddress('npub',pubkey);
				}
				if (networks[DOM.network.val()].name == "Nostr - Notes and Other Stuff Transmitted by Relays") {
					if (hasPrivkey) {
                        privkey = libs.ethUtil.bufferToHex(keyPair.d.toBuffer());
						privkey = getaddress('nsec',privkey);
                    }
				}
                //TRX is different
    ...
    var networks = [
        ...
        },
        {
            ...
            onSelect: function() {
                network = libs.bitcoin.networks.waykichain;
                setHdCoin(99999);
            },
        },{
            name: "AC - Asiacoin",
